...
namespace PixelCrushers.DialogueSystem
{
    ...
    public class ProximitySelector : MonoBehaviour
    {
        ...
        private List<Usable> allUsables = new List<Usable>();
        private int closestUsableIndex = 0;
        ...
        public virtual void Start()
        {
            ...
            allUsables = GameObject.FindObjectsOfType<Usable>().ToList();
        ...
        protected virtual void Update()
        {
            ...
            OnClosestUsableObject();
            
        }

        protected void OnClosestUsableObject()
        {
            var allUsableArr = allUsables.ToArray();
            closestUsableIndex = GetClosestObjectIndex(ref allUsableArr, gameObject);
            Usable closestUsable = allUsables[closestUsableIndex];
            if (Vector2.Distance(transform.position, closestUsable.gameObject.transform.position) <= maxSelectionDistance)
            {
                CheckTriggerEnter(closestUsable.gameObject);

                if (IsClicked(closestUsable)) UseCurrentSelection();
            }
            else RemoveUsableFromDetectedList(closestUsable);
        }

        protected bool IsClicked(Usable usable) 
        {
            return usable.OnClick();
        }
